...
outputDir = fullfile('..', 'images_stego');
...
[~, name, ext] = fileparts(fullfile('..', 'images_cover', '1.pgm'));

% Create output filename with payload information
outputFilename = fullfile(outputDir, sprintf('%s_stego_%.2fbpp%s', name, payload, ext));

% Save the stego image
stego_uint8 = uint8(stego);
imwrite(stego_uint8, outputFilename, 'pgm');
