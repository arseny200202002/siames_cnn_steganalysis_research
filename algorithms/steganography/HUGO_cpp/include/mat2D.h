...
public:
	...
	//					STATIC METHODS
	// -----------------------------------------------------
	static mat2D<double>* Retype_Double(mat2D<T> *mat)
	{
	...
	static mat2D<T>* AbsoluteValue(mat2D<T> *mat)
	{
		...
		for (int rows=0; rows < mat->rows; rows++)
			for (int cols=0; cols < mat->cols; cols++)
				result->Write(rows, cols, abs(mat->Read(rows, cols)));
		return result;
	}
	...
	static std::vector<mat2D<T> *> Correlation_Same_basicFilters(mat2D<T> * mat, mat2D<mat2D<T> *> * basicFilters)
	{
		...
		{
			mat2D<double> * temp1 = mat2D::Correlation_Same(mat, basicFilters->Read(filterIndex, 0));
			mat2D<double> * temp2 = mat2D::Correlation_Same(temp1, basicFilters->Read(filterIndex, 1));
			delete temp1;
