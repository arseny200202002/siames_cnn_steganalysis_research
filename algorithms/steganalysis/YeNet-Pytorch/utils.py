...
class RandomBalancedSampler(Sampler):
    ...
    def __iter__(self):
        # data_source имеет длину 2 * количество cover (сначала все cover, потом все stego)
        n_pairs = self.num_samples // 2
        # Генерируем случайный порядок для пар
        perm = torch.randperm(n_pairs).tolist()
        # Создаем список индексов, чередующих cover и stego
        indices = []
        for i in perm:
...
class DataLoaderStego(DataLoader):
    def __init__(self, cover_dir, stego_dir, embedding_otf=False,
        ...
        # Выбираем соответствующий датасет
        if pair_constraint and batch_size % 2 == 0:
            dataset = DatasetPair(cover_dir, stego_dir, embedding_otf, transform)
            _batch_size = batch_size // 2  # целочисленное деление
        else:
            dataset = DatasetNoPair(cover_dir, stego_dir, embedding_otf, transform)
            _batch_size = batch_size
        
        # Выбираем сэмплер
        if pair_constraint:
            sampler = torch.utils.data.RandomSampler(dataset) if shuffle else \
                      torch.utils.data.SequentialSampler(dataset)
        else:
            sampler = RandomBalancedSampler(dataset)
        
        # Создаём DataLoader с кастомной collate_fn
        super(DataLoaderStego, self).__init__(
            ...
            drop_last=True,
            collate_fn=self.collate_with_reshape
        )
        
        self.shuffle = shuffle
    
    def collate_with_reshape(self, batch):
        """
        Кастомная функция для объединения батча и изменения формы.
        """
        # Стандартное объединение в батч
        images = torch.stack([item['images'] for item in batch])
        labels = torch.stack([item['labels'] for item in batch])
        
        # Изменяем форму: (batch_size, 2, C, H, W) -> (batch_size*2, C, H, W)
        if self.pair_constraint:
            shape = list(images.shape)
            # batch_size, 2, C, H, W -> (batch_size*2, C, H, W)
